...
def move(i, mode):
    for j in range(3): # 반복횟수는 블록을 몇 번만에 움직이도록 할 것인가를 결정
        x[i]=x[i] + (20*(mode//2)-30)*(mode//3)*3  # 10*3만큼씩 움직인다고 설정한 것
        y[i]=y[i] + (20*mode-30)*(1-mode//3)*3     # 즉, 30만큼 총 3번에 걸쳐서 움직이게 됨(총 90만큼 이동)
        rect(x[i],y[i],T[i],W[i],i)

# mode에 따라 블록의 움직임을 결정할 수 있음
# 1번에 10만큼씩 움직임(좀 더 빠르게 블록을 움직이고 싶다면, 움직이는 거리를 증가시키면 되는데...
# 이때, 위의 def move 함수 내에 있는 반복문의 반복 횟수를 함께 조정해야 함.
# 즉, 블록의 길이가 80이고, 그 사이가 10이면 블록이 움직여야 하는 길이는 총 90임
# 10만큼씩 90을 움직이려면 총 반복을 9번 해야하는데.. 이를 좀 더 빨리 움직이도록 하고 싶다면..
# 10*3만큼씩 90을 움직이게 하면 됨. 즉 3번을 반복하여 움직이도록 하면됨.
